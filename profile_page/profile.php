...
<link rel="stylesheet" type="text/css" href="./styling_profile.css">
...
<body class="font-monospace">
      ...
      <div class="collapse navbar-collapse" id="navbarTogglerDemo02">
        ...
        <li><?php
            if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {
                echo '<a class="nav-link rounded" href="./profile.php" aria-expanded="false">';
  ...
  if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {
      ...
      // Check if the user does not have reviewer permission
      if ($_SESSION['permission'] === 0) {
          // Calculate total reviews from the reviews table
          $sql_reviews_count = "SELECT COUNT(*) as total_reviews FROM reviews WHERE TRIM(reviewer_name) = '$user_id'";
          $result_reviews_count = mysqli_query($conn, $sql_reviews_count);

          if ($result_reviews_count && mysqli_num_rows($result_reviews_count) > 0) {
      ...
      $user_id = 'johndoe';
      $email = 'johndoe@bird.com';
  }

  echo '<div class="container profil my-4">
      <div class="card" style="width: 18rem;">
          <img src="../misc/guest_picture.jpg" class="card-img-top rounded" alt="Profile Picture">
          <div class="card-body text-center">
              <h5 class="card-title text-center"><strong>' . $user_id . '</strong></h5>
              <h6 class="card-text">Email: <b>' . $email . '</b></h6>';

  if ($_SESSION['permission'] === 0) {
      echo '<small class="card-text">Total reviews: <b>' . $total_reviews . '</b></small><br>';
  }

  echo '<div class="btn-group my-2 btn-group-sm" role="group" aria-label="Basic radio toggle button group">
        <input type="radio" class="btn-check" name="btnradio" id="btnradio1" autocomplete="off"
            ' . (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true && $_SESSION['permission'] === 1 ? 'checked' : '') . ' disabled>
        <label class="btn btn-outline-dark btn" for="btnradio1">Reviewer</label><br>

        <input type="radio" class="btn-check" name="btnradio" id="btnradio2" autocomplete="off"  
            ' . (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true && $_SESSION['permission'] === 0 ? 'checked' : '') . ' disabled>
        <label class="btn btn-outline-dark" for="btnradio2">User</label>
    </div>';

  if ($_SESSION['permission'] === 0) {
      echo '<form action="../php_scripts/logout_script.php" method="post">
        <button class="btn btn-sm btn-outline-danger mx-auto" type="submit">Sign Out</button>
    </form>';
  }
  ...
  if ($_SESSION['permission'] === 0) {
      echo '<form action="../php_scripts/add_request.php" method="post">';

      echo '<div class="container">';
      // Add the card container with mx-auto class for horizontal centering
      echo '<div class="card mx-auto mb-3" style="width: 18rem;">';

      // Add the card body
      echo '<div class="card-body text-center d-flex flex-column justify-content-between">';

      // Add the textarea inside the card body
      echo '<div class="mb-3">';
      echo '<textarea class="form-control" id="request_reason" name="request_reason" rows="4" required>State your reason here:</textarea>';
      echo '</div>';

      // Add the button inside the card body
      echo '<button class="btn btn-outline-success btn-lg mx-auto" type="submit">Request to become a reviewer!</button>';

      // Close the card body and card container
      echo '</div>';
      echo '</div>';
